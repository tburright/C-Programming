...
; 32:											 edi, esi, edx, ecx, ebx, eax

	xor eax, eax
	mov ebx, 0				; iterator

	mov esi, [esp + 4]		; string that has been shl
	mov edi, [esp + 8]		; empty null term char string
	mov ecx, [esp + 12]		; size of strings

	.loopy:
		lodsb				; load string bytes from esi into eax
		shr eax, 1
		stosb				; store from eax into edi
		inc ebx				; i++
		cmp ebx, ecx
		jne .loopy
...
; mov edi, [edi + Node.data]
; recieves args into: ecx, edx, ...

	xor eax, eax
	xor edi, edi
	xor esi, esi

	mov edi, [ecx]			; param 1: Node

	.loopy:
		mov esi, [edi + Node.Data]	; Set esi to current nodes data
		cmp esi, edx				; Compare current node data to target needle (param 2)
		je .found					; Found it! jump to .found label
		mov edi, [edi + Node.Next]	; Didn't find needle. Move to next node
		cmp edi, 0					; Check if next node is null
		je .end						; -if so, rage quit
		jmp .loopy
...
call _first_func@0		; No arguments. returns into eax?
push eax				; Push for _test_func arg?

call _test_func			; Call with pushed eax arg? returns into eax?
pop ebx					; don't care but needs to be popped off?
ret
